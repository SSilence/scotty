...
public class URLFetcher implements HTTPClient {
	...
	/**
	 * Can be used by a calling class to fetch a request without spawning an
	...
	public Response fetchResponse(Request request) throws IOException {
		...
		if (keyFingerprint == null && _keyFingerprint == null) {
			// no problem
		} else if (keyFingerprint != null && _keyFingerprint != null
				&& keyFingerprint.equals(_keyFingerprint)) {
			// no problem
		} else {
		...
		if (_proxyAuthCreds == null && _authenticator != null && useProxy(url))
			_proxyAuthCreds = _authenticator.getProxyCredentials(url.toString()
					.startsWith("https") ? _httpsProxy : _httpProxy, null);
		String proxyAuthHeader = constructAuthenticationHeader(null,
				_proxyAuthCreds);
		...
		do {
			...
			if (_direct) {
				...
				if (proxyAuthHeader != null) {
					...
					if (proxyAuthHeader.startsWith("NTLM")
							|| proxyAuthHeader.startsWith("Negotiate")) {
						if (request.getVersion().equals("HTTP/1.0")) {
							// we have to explicitly tell the server to keep the
							// connection alive for 1.0
							request.setHeader("Connection", "Keep-Alive");
			...
			if (status.equals("407")) {
				...
				String[] challenges = _response
						.getHeaders("Proxy-Authenticate");
				if (_proxyAuthCreds == null && _authenticator != null) {
					_proxyAuthCreds = _authenticator.getProxyCredentials(
							_httpProxy, challenges);
				}
				proxyAuthHeader = constructAuthenticationHeader(challenges,
						_proxyAuthCreds);
				if (proxyAuthHeader != null && oldProxyAuthHeader != null
						&& oldProxyAuthHeader.equals(proxyAuthHeader)) {
			...
			if (status.equals("401")) {
				...
				authHeader = constructAuthenticationHeader(challenges,
						_authCreds);
				_logger.finer("Auth header is " + authHeader);
				if (authHeader != null && oldAuthHeader != null
						&& oldAuthHeader.equals(authHeader)) {
					_logger.info("No possible authentication");
					authHeader = null;
				}
			}
			...
			tries++;
				...
				&& ((status.equals("401") && authHeader != null) || (status
						.equals("407") && proxyAuthHeader != null)));
	...
	private void connect(HttpUrl url) throws IOException {
		...
		if (useProxy(url)) {
			if (!ssl) {
				...
				_socket.connect(new InetSocketAddress(_httpProxy,
						_httpProxyPort), _connectTimeout);
				_in = _socket.getInputStream();
				_out = _socket.getOutputStream();
				_direct = false;
			} else {
				_socket.connect(new InetSocketAddress(_httpsProxy,
						_httpsProxyPort), _connectTimeout);
				_in = _socket.getInputStream();
				_out = _socket.getOutputStream();
				String oldAuthHeader = null;
				String authHeader = constructAuthenticationHeader(null,
						_proxyAuthCreds);
				String status;
				do {
					...
					response.read(_in);
					_logger.fine("Got proxy response "
							+ response.getStatusLine());
					status = response.getStatus();
					if (status.equals("407")) {
						response.flushContentStream();
						oldAuthHeader = authHeader;
						String[] challenges = response
								.getHeaders("Proxy-Authenticate");
						if (_proxyAuthCreds == null && _authenticator != null)
							_proxyAuthCreds = _authenticator
									.getProxyCredentials(_httpsProxy,
											challenges);
						if (_proxyAuthCreds == null) {
							_response = response;
							return;
		...
		if (ssl) {
			...
			SSLContext sslContext = _sslContextManager
					.getSSLContext(_keyFingerprint);
			if (sslContext == null)
				throw new IOException(
						"No SSL cert found matching fingerprint: "
								+ _keyFingerprint);
			// Use the factory to create a secure socket connected to the
			// HTTPS port of the specified web server.
			try {
				SSLSocketFactory factory = sslContext.getSocketFactory();
				SSLSocket sslsocket = (SSLSocket) factory.createSocket(_socket,
						_socket.getInetAddress().getHostName(),
		...
		if (ssl && "".equals(_httpsProxy)) {
			...
			for (int i = 0; i < _noProxy.length; i++) {
				if (_noProxy[i].startsWith(".") && host.endsWith(_noProxy[i])) {
					return false;
				} else if (_noProxy[i].equals("<local>")
						&& host.indexOf('.') < 0) {
					return false;
		...
		if (url.getHost().equals(_host)) {
			...
			if (urlport == _port) {
				...
				if (now - _lastRequestTime > 1000) {
					_logger.fine("Socket has expired ("
							+ (now - _lastRequestTime) + "), open a new one!");
					return true;
